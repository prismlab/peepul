...
val dequeue : s1:s
            -> Pure ((option (nat * nat)) * s)
              (requires true)
              (ensures (fun (_, r) -> (forall e. memq e r <==> memq e s1 /\ Some e <> peek s1) /\
...
val sim01 : tr:ae
         -> s0:s
         -> Tot (b:bool{b = true <==> (forall (e e1:(nat * nat)). ((mem (fst e, Enqueue (snd e)) tr.l /\ mem (fst e1, Enqueue (snd e1)) tr.l /\ fst e <> fst e1 /\
...
let sim01 tr s0 =
    ...
    forall_mem enq_list (fun e1 -> mem e1 enq_list &&
                  (forall_mem (filter_op (fun e0 -> mem e0 enq_list && is_enqueue e0 && get_id e0 <> get_id e1 &&
                                            (tr.vis e1 e0 || (not (tr.vis e1 e0 || tr.vis e0 e1) &&
                                         (get_ele e1 < get_ele e0 || (get_ele e1 = get_ele e0 && get_id e1 < get_id e0))))) enq_list)
                              (fun e2 -> mem e2 enq_list && mem ((get_id e1), (get_ele e1)) (tolist s0) && mem ((get_id e2), (get_ele e2)) (tolist s0) &&
                                        get_id e1 <> get_id e2 && (order ((get_id e1), (get_ele e1)) ((get_id e2), (get_ele e2)) (tolist s0)))
                                    ))

val sim1 : tr:ae
        -> s0:s
        -> Tot(b:bool{b = true <==> (forall (e1 e2:(nat * nat)). (memq e1 s0 /\ memq e2 s0 /\ get_id e1 <> get_id e2 /\ order e1 e2 (tolist s0)) <==>
...
let sim1 tr s0 =
    ...
    let enq_list = filter_op (fun x -> is_enqueue x && mem x tr.l &&
                                    not (exists_mem tr.l (fun d -> is_dequeue d && mem d tr.l && mem x tr.l && get_id x <> get_id d && matched x d tr))) tr.l in
    (forall_mem (tolist s0) (fun x -> memq x s0 &&
                  (forall_mem (filter_s (fun z -> fst x <> fst z && memq x s0 && memq z s0 && order x z (tolist s0)) (tolist s0))
...
val sim : tr:ae
        ...
        -> Tot(b:bool{b = true <==>
                       ((forall e. memq e s0 <==> (exists e1. mem e1 tr.l /\ is_enqueue e1 /\ e = (get_id e1, get_ele e1) /\
                           ...
                           ) /\ (forall (e1 e2:(nat * nat)). (memq e1 s0 /\ memq e2 s0 /\ get_id e1 <> get_id e2 /\ order e1 e2 (tolist s0)) <==>
                          (mem (fst e1, Enqueue (snd e1)) tr.l /\ mem (fst e2, Enqueue (snd e2)) tr.l /\ (fst e1 <> fst e2) /\
                                 (forall d. mem d tr.l /\ fst e1 <> get_id d /\ fst e2 <> get_id d /\ is_dequeue d ==>
                                 (not (matched (fst e1, Enqueue (snd e1)) d tr)) /\ (not (matched (fst e2, Enqueue (snd e2)) d tr))) /\
                                (tr.vis (fst e1, Enqueue (snd e1)) (fst e2, Enqueue (snd e2)) \/
...
let rec sorted l = match l with
    | [] -> true
    | [x] -> true
    | x::y::xs -> ((snd x < snd y) || (snd x = snd y && fst x < fst y)) && (sorted (y::xs))

val sorted_smaller : x:(nat * nat)
                   ->  y:(nat * nat)
                   ->  l:list (nat * nat)
                   ->  Lemma (requires (unique_id (x::l) /\ sorted (x::l) /\ mem y l))
                            (ensures ((snd x < snd y) || (snd x = snd y && fst x < fst y)))
                            [SMTPat (sorted (x::l)); SMTPat (mem y l)]
let rec sorted_smaller x y l = match l with
    | [] -> ()
    | z::zs -> if z=y then () else sorted_smaller x y zs

val insert : i:(nat * nat)
           -> l:list (nat * nat) {not (mem_id (fst i) l) /\ unique_id l /\ sorted l}
           -> Tot (m:list (nat * nat ){unique_id m /\ sorted m /\ (forall x. mem x m <==> (x = i \/ mem x l))})
let rec insert i l = match l with
  | [] -> [i]
  | hd::tl -> if ((snd i < snd hd) || (snd i = snd hd && fst i < fst hd)) then i::l else
            let i_tl = insert i tl in
            let (z::_) = i_tl in
            if mem z tl then sorted_smaller hd z tl; hd::i_tl

val sort : l:list (nat * nat) {unique_id l}
         -> Tot (m:list (nat * nat) {unique_id m /\ sorted m /\ (forall x. mem x l <==> mem x m)})
...
val union1 : a:list (nat * nat)
        -> b:list (nat * nat)
        -> Pure (list (nat * nat))
               (requires (unique_id a /\ unique_id b) /\ (forall e. mem e a ==> not (mem_id (fst e) b)) /\ (sorted a) /\ sorted b)
...
let rec union1 l1 l2 =
        ...
        | [], l2 -> l2
        | l1, [] -> l1
        | h1::t1, h2::t2 -> if (snd h1 < snd h2) || (snd h1 = snd h2 && fst h1 < fst h2)
...
val merge_s : l:list (nat * nat)
                ...
                -> Pure (list (nat * nat))
                  ...
                  (ensures (fun res -> unique_id res /\ (forall e. mem e res <==> ((mem e l /\ mem e a /\ mem e b) \/
                                    ...
                                    (forall e e1. (mem e (diff_s a l) /\ mem e1 (diff_s b l) /\ (snd e < snd e1 \/ (snd e = snd e1 /\ fst e < fst e1)))
                                         ==> order e e1 res)
    ))
...
val merge : ltr:ae
          ...
          -> Pure s (requires ((forall e. mem e ltr.l ==> not (member (get_id e) atr.l)) /\
                               ...
                               (forall e e1. mem e ltr.l /\ mem e (union atr btr).l /\ get_id e <> get_id e1 ==> (absmerge ltr atr btr).vis e e1)))
...
let prop_merge ltr l atr a btr b = prop_merge0 ltr l atr a btr b; prop_merge1 ltr l atr a btr b; ()
