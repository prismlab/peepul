...
let rec random_ops r1 r2 r_l = let r = Random.int 100 in
  if (r < r1) then
    begin
      ...
      if Z.equal ele (Z.of_int (-1)) then (random_ops (-1) 101 r_l) else
        ((ele, N.Look ele), r_l)
    end
  else
    begin
...
let app_op set (op, l) =
  ...
  max_size := max (size (M.flatten res)) (!max_size);
  (res, l)
...
let rec test lca a b a_l b_l count =
  ...
  let ((a, a_l), (b, b_l)) = if pick_r replica_ratio a b = a then (app_op a (random_ops (-1) 70 a_l), (b, b_l))
    else ((a, a_l), app_op b (random_ops (-1) 70 b_l)) in
...
let rec gen_lca p p_l count =
  if count = 0 then (p, p_l) else
    ...
    let (p, p_l) = app_op p (random_ops (-1) 70 p_l) in
    gen_lca p p_l (count - 1)

let _ =
  let (lca, lca_l) = gen_lca (M.totree []) [] 1000 in
  let a = lca in
  let b = lca in
