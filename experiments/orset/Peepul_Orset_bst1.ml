...
let (l : (Prims.int * Prims.int) Prims.list) =
  [((Prims.of_int (3)), (Prims.of_int (3)));
  ((Prims.of_int (2)), (Prims.of_int (2)))]
let (check :
  (Prims.nat * Prims.nat) Prims.list -> (Prims.nat * Prims.nat) Prims.list) =
  fun l1 ->
    FStar_List_Tot_Base.sortWith (fun a -> fun b -> (snd b) - (snd a)) l1
let (l' : (Prims.nat * Prims.nat) Prims.list) =
  check
    [((Prims.of_int (3)), (Prims.of_int (3)));
...
let (takemiddle : Orset_opt.s -> Orset_opt.s) =
  fun l1 ->
    take_element l1 ((FStar_List_Tot_Base.length l1) / (Prims.of_int (2)))
let rec (take : Prims.nat -> Orset_opt.s -> Orset_opt.s) =
  fun n ->
    fun l1 ->
      if n = Prims.int_zero
      then []
      else (match l1 with | h::t1 -> h :: (take (n - Prims.int_one) t1))

let (takefront : Orset_opt.s -> Orset_opt.s) =
  fun l1 ->
    let t1 = take ((FStar_List_Tot_Base.length l1) / (Prims.of_int (2))) l1 in
    t1
let rec (drop : Prims.nat -> Orset_opt.s -> Orset_opt.s) =
  fun n ->
    fun l1 ->
      if n = Prims.int_zero
      then l1
      else (match l1 with | h::t1 -> drop (n - Prims.int_one) t1)
let (takeback : Orset_opt.s -> Orset_opt.s) =
  fun l1 ->
    drop
      (((FStar_List_Tot_Base.length l1) / (Prims.of_int (2))) + Prims.int_one)
      l1
let rec (tree_of_list : Orset_opt.s -> tree) =
  fun l1 ->
    ...
    | h::t1 ->
        Node
          ((FStar_List_Tot_Base.hd (takemiddle l1)),
            (tree_of_list (takefront l1)), (tree_of_list (takeback l1)))


let (sort :
  (Prims.nat * Prims.nat) Prims.list -> (Prims.nat * Prims.nat) Prims.list) =
  fun lst ->
    FStar_List_Tot_Base.sortWith (fun x -> fun y -> (snd x) - (snd y)) lst
      ...
      fun atr ->
        fun a ->
          fun btr ->
            fun b ->
              let m =
                Orset_opt.merge ltr (flatten l1) atr (flatten a) btr
                  (flatten b) in
              let m' =
                FStar_List_Tot_Base.sortWith
                  (fun x -> fun y -> (snd x) - (snd y)) m in
              tree_of_list m'
