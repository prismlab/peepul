...
let rec (totree1 : Peepul_Orset_opt.s -> t -> t) =
  fun l ->
    fun acc -> match l with | [] -> acc | x::xs -> totree1 xs (insert x acc)
let (totree : Peepul_Orset_opt.s -> t) = fun l -> totree1 l Leaf
let (lt : (Prims.nat * Prims.nat) -> (Prims.nat * Prims.nat) -> Prims.bool) =
  fun uu___ ->
    fun uu___1 ->
      match (uu___, uu___1) with
      | ((id, ele), (id1, ele1)) -> (ele < ele1) && (id <> id1)
let rec (appendt : Peepul_Orset_opt.s -> Peepul_Orset_opt.s -> Peepul_Orset_opt.s) =
  fun l1 ->
    fun l2 ->
      match (l1, l2) with
      | ([], []) -> []
      ...
      | ([], uu___) -> l2
let rec (flatten : t -> Peepul_Orset_opt.s) =
  fun t1 ->
    match t1 with
    | Leaf -> []
...
let (sim : Peepul_Orset_opt.ae -> t -> Prims.bool) =
  fun tr ->
    fun s1 ->
      let lsta =
        Peepul_Orset_opt.filter (fun a -> Peepul_Orset_opt.opa a)
          (Peepul_Orset_opt.__proj__A__item__l tr) in
      let lstr =
        Peepul_Orset_opt.filter (fun r -> Peepul_Orset_opt.opr r)
          (Peepul_Orset_opt.__proj__A__item__l tr) in
      let lst =
        Peepul_Orset_opt.filter
          (fun a ->
             Prims.op_Negation
               (Peepul_Orset_opt.existsb
                  (fun r ->
                     (((Peepul_Orset_opt.get_id a) <> (Peepul_Orset_opt.get_id r)) &&
                        ((Peepul_Orset_opt.get_ele r) = (Peepul_Orset_opt.get_ele a)))
                       && (Peepul_Orset_opt.__proj__A__item__vis tr a r)) lstr))
          lsta in
      (forallt
         (fun e ->
            (Peepul_Orset_opt.forallb (fun a -> (fst e) >= (Peepul_Orset_opt.get_id a))
               (Peepul_Orset_opt.filter (fun a -> (Peepul_Orset_opt.get_ele a) = (snd e))
                  lst))
              &&
              ((FStar_List_Tot_Base.mem ((fst e), (Peepul_Orset_opt.Add (snd e)))
                  (Peepul_Orset_opt.__proj__A__item__l tr))
                 &&
                 (Prims.op_Negation
                    (Peepul_Orset_opt.existsb
                       (fun r ->
                          ((fst e) <> (Peepul_Orset_opt.get_id r)) &&
                            (Peepul_Orset_opt.__proj__A__item__vis tr
                               ((fst e), (Peepul_Orset_opt.Add (snd e))) r))
                       (Peepul_Orset_opt.filter
                          (fun r -> (snd e) = (Peepul_Orset_opt.get_ele r)) lstr)))))
         s1)
        &&
        (Peepul_Orset_opt.forallb (fun a -> member_ele (Peepul_Orset_opt.get_ele a) s1) lst)
let (diff : t -> t -> t) =
  fun a -> fun l -> totree (Peepul_Orset_opt.diff (flatten a) (flatten l))
(* let (merge : *)
(*   Peepul_Orset_opt.ae -> t -> Peepul_Orset_opt.ae -> t -> Peepul_Orset_opt.ae -> t -> t) = *)
(*   fun ltr -> *)
(*     fun l -> *)
(*       fun atr -> *)
(*         fun a -> *)
...
(*             fun b -> *)
(*               let m = *)
(*                 Peepul_Orset_opt.merge ltr (flatten l) atr (flatten a) btr *)
(*                   (flatten b) in *)
(*               totree m *)
...
let (merge1 : t ->  t -> t -> t) =
  ...
  fun b ->
  let m =
    Peepul_Orset_opt.merge1 (flatten l) (flatten a) (flatten b) in
  totree m

let (max : Prims.int -> Prims.int -> Prims.int) =
  fun n1 -> fun n2 -> if n1 > n2 then n1 else n2
let rec (pos : Peepul_Orset_opt.s -> (Prims.nat * Prims.nat) -> Prims.nat) =
  fun l ->
    fun e ->
      match l with
      | x::y -> if x = e then Prims.int_zero else Prims.int_one + (pos y e)
let rec (sorted : Peepul_Orset_opt.s -> Prims.bool) =
  fun l ->
    match l with
    | [] -> true
    | x::[] -> true
    | x::y::xs -> ((snd x) < (snd y)) && (sorted (y :: xs))
let rec (take_element : Peepul_Orset_opt.s -> Prims.nat -> Peepul_Orset_opt.s) =
  fun l ->
    fun n ->
      match l with
      | h::t1 ->
...
let (takemiddle : Peepul_Orset_opt.s -> Peepul_Orset_opt.s) =
  fun l ->
    take_element l ((FStar_List_Tot_Base.length l) / (Prims.of_int (2)))
let rec (take : Prims.nat -> Peepul_Orset_opt.s -> Peepul_Orset_opt.s) =
  fun n ->
    fun l ->
      if n = Prims.int_zero
      then []
...
let (takefront : Peepul_Orset_opt.s -> Peepul_Orset_opt.s) =
  fun l ->
    let t1 = take ((FStar_List_Tot_Base.length l) / (Prims.of_int (2))) l in
    t1
let rec (drop : Prims.nat -> Peepul_Orset_opt.s -> Peepul_Orset_opt.s) =
  fun n ->
    fun l ->
      if n = Prims.int_zero
      then l
      else (match l with | h::t1 -> drop (n - Prims.int_one) t1)
let (takeback : Peepul_Orset_opt.s -> Peepul_Orset_opt.s) =
  fun l ->
    drop
      (((FStar_List_Tot_Base.length l) / (Prims.of_int (2))) + Prims.int_one)
      l
let rec (tree_of_list : Peepul_Orset_opt.s -> tree) =
  fun l ->
    match l with
    | [] -> Leaf
    | h::[] -> Node (h, Leaf, Leaf)
