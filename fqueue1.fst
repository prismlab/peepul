...
val get_val : a:option (nat * nat){Some? a} -> n:(nat * nat){exists pair. a = Some pair}
...
val dequeue : s1:s
            -> Pure ((option (nat * nat)) * s)
              ...
              (ensures (fun r -> (forall e. memq e (snd r) <==> memq e s1 /\ Some e <> peek s1) /\
                   ...
                   (s1.ls <> [] ==> (forall e e1. mem e (snd r).ls /\ mem e1 (snd r).ls /\ fst e <> fst e1 /\ order e e1 (snd r).ls <==> mem e s1.ls /\ mem e1 s1.ls /\ fst e <> fst e1 /\ e <> get_val (fst r) /\ e1 <> get_val (fst r) /\ order e e1 (tolist s1))) /\
                   (s1.ls = [] ==> (forall e e1. mem e (snd r).ls /\ mem e1 (snd r).ls /\ fst e <> fst e1 /\ order e e1 (snd r).ls <==> mem e s1.ls /\ mem e1 s1.ls /\ fst e <> fst e1 /\ order e e1 (tolist s1))) /\
                     (fst r = peek s1)))
let dequeue q =
  match q with
  |(S []) -> (None, q)
...
val app_op : s1:s
           -> op:o
           -> Pure s
             (requires (not (mem_id (get_id op) s1.ls)) /\ (is_dequeue op ==> return op = peek s1))
             (ensures (fun r ->
                         ...
                         (s1.ls <> [] ==> (length r.ls = length s1.ls - 1) /\ peek s1 <> None) /\ 
                         (s1.ls = [] ==> length r.ls = length s1.ls) /\
                         (s1.ls <> [] ==> (forall e e1. mem e r.ls /\ mem e1 r.ls /\ fst e <> fst e1 /\ order e e1 r.ls <==> mem e s1.ls /\ mem e1 s1.ls /\ fst e <> fst e1 /\ e <> get_val (peek s1) /\ e1 <> get_val (peek s1) /\ order e e1 (tolist s1))) /\
                         (s1.ls = [] ==> (forall e e1. mem e r.ls /\ mem e1 r.ls /\ fst e <> fst e1 /\ order e e1 r.ls <==> mem e s1.ls /\ mem e1 s1.ls /\ fst e <> fst e1 /\ order e e1 (tolist s1))))) /\
                         (exists n. get_op op = (Enqueue n) ==> (exists id. rear r = (Some (id,n)))) /\
                         (s1.ls <> [] /\ is_dequeue op ==> not (mem (get_val (peek s1)) r.ls)) /\
...
assume val axiom_queue_ae : tr:ae
                 ...
                 (forall e d1. mem e tr.l /\ mem d1 tr.l /\ is_dequeue d1 /\ return d1 = None /\
                       is_enqueue e /\ tr.vis e d1 ==> (exists d2. mem d2 tr.l /\ get_id e <> get_id d2 /\ get_id d1 <> get_id d2 /\ tr.vis d2 d1 /\ matched e d2 tr)) /\
        (forall e1 e2 d. mem e1 tr.l /\ mem e2 tr.l /\ mem d tr.l /\ is_enqueue e1 /\  get_id e2 <> get_id d /\ matched e2 d tr
                        /\ tr.vis e1 e2 ==> (exists d1. mem d1 tr.l /\  get_id e1 <> get_id d1 /\ matched e1 d1 tr)) /\
                               (forall e1 e2 d1 d2. mem e1 tr.l /\ mem e2 tr.l /\ mem d1 tr.l /\ mem d2 tr.l /\  get_id e1 <> get_id d2 /\  get_id e2 <> get_id d1 ==> not (matched e1 d2 tr && matched e2 d1 tr && tr.vis e1 e2 && tr.vis d1 d2))))
              [SMTPat (unique tr.l)]
...
val eff_enq : tr:ae
            ...
            (ensures (fun r -> forall_mem r (fun x -> is_enqueue x && mem x tr.l && not
                                     (exists_mem tr.l (fun d -> is_dequeue d && mem d tr.l && mem x tr.l && get_id x <> get_id d && matched x d tr)))))
let eff_enq tr = filter_op (fun x -> is_enqueue x && mem x tr.l && not
                                     (exists_mem tr.l (fun d -> is_dequeue d && mem d tr.l && mem x tr.l && get_id x <> get_id d && matched x d tr))) tr.l
