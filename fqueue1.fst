...
type s =
    |S : l : list (nat (* UID *) * nat (* value of the element *)) {unique_id l}
       -> s

val memq : n:(nat * nat) -> q:s -> Tot (b:bool{b = true <==> (mem n q.l)})
let memq n q = (mem n q.l)
...
val tolist : s1:s
           -> Pure (list (nat * nat))
                  ...
                  (ensures (fun r -> (forall e. mem e r <==> memq e s1) /\ unique_id r /\
                           (forall e e1. (mem e s1.l /\ mem e1 s1.l /\ fst e <> fst e1 /\ order e e1 s1.l) <==> mem e r /\ mem e1 r /\ fst e <> fst e1 /\ order e e1 r)))
let tolist (S l) = l
...
val peek : s1:s
         -> Pure (option (nat * nat))
                (requires true)
                (ensures (fun r -> ((norm s1).l = [] ==> r = None) /\
                                ((norm s1).l <> [] ==> (exists id n. r = Some (id, n)))))
let peek q =
  let n = norm q in
  match n with
  |(S []) -> None
...
val rear : s1:s
         -> Pure (option (nat * nat))
                (requires true)
                (ensures (fun r -> (s1.l = [] ==> r = None) /\
                                (s1.l <> [] ==> (exists id n. r = Some (id, n))) /\
                                (s1.l <> [] ==> (exists x. r = Some (last_ele x)))))
let rear q =
  match q with
  |(S []) -> None
  |(S x) -> Some (last_ele x)
...
val enqueue : x:(nat * nat)
            -> s1:s
            -> Pure s
             (requires (not (mem_id (fst x) s1.l)))
             (ensures (fun r -> (rear r = Some x) /\ (forall e. memq e s1 \/ e = x <==> memq e r) /\
                      (forall e e1. mem e s1.l /\ mem e1 s1.l /\ fst e <> fst e1 /\ order e e1 s1.l ==> order e e1 (tolist r)) /\
...
val enqueue0 :x:(nat * nat)
             -> s1:s
             -> Lemma (requires (not (mem_id (fst x) s1.l)))
...
val is_empty : s1:s -> Tot (b:bool{(s1.l = []) <==> b = true})
let is_empty s = (s.l = [])
...
val dequeue : s1:s
            -> Pure ((option (nat * nat)) * s)
              ...
              (ensures (fun (v, r) -> (forall e. memq e r <==> memq e s1 /\ Some e <> peek s1) /\
                          (forall e e1. mem e r.l /\ mem e1 r.l /\ fst e <> fst e1 /\ order e e1 r.l ==> order e e1 (tolist s1)) /\
...
val app_op : s1:s
           -> op:o
           -> Pure s
             (requires ((not (mem_id (get_id op) s1.l))))
             (ensures (fun r ->
                         (is_enqueue op ==> (rear r = Some (get_id op, get_ele op)) /\ (forall e. memq e s1 \/ e = (get_id op, get_ele op) <==> memq e r) /\
                                           ...
                                           (forall e e1. (memq e s1 /\ fst e <> fst e1 /\ ((memq e1 s1 /\ order e e1 (tolist s1)) \/ (e1 = (get_id op, get_ele op)))) <==>
                                           (memq e r /\ memq e1 r /\ fst e <> fst e1 /\ order e e1 (tolist r)))) /\
...
let is_empty' l s1 = ((length s1.l) + (len_del l) = 0)
...
let prop_merge0 ltr l atr a btr b =
  ...
  assert(forall e. memq e s0 ==> mem (fst e, Enqueue (snd e)) enq_list);
  assert(forall_mem (tolist s0) (fun e -> mem (fst e, Enqueue (snd e)) enq_list) &&
                    forall_mem enq_list (fun x -> mem x tr.l && is_enqueue x && mem ((get_id x), (get_ele x)) (tolist s0)));
  ()
...
let prop_merge2 ltr l atr a btr b =
  ...
  prop_merge215 ltr l atr a btr b; prop_merge216 ltr l atr a btr b; prop_merge216 ltr l atr a btr b; prop_merge218 ltr l atr a btr b;
  let tr = absmerge ltr atr btr in
  let s0 = merge ltr l atr a btr b in
  let enq_list = filter_op (fun x -> is_enqueue x && mem x tr.l && not
                             (exists_mem tr.l (fun d -> is_dequeue d && mem d tr.l && mem x tr.l && get_id x <> get_id d && matched x d tr))) tr.l in
  ...
  assert(forall e. mem e enq_list ==> mem e enq_list4 \/ mem e enq_list5 \/ mem e enq_list6);
  assert(forall e e1. mem e (enq_list) /\ mem e1 (enq_list) /\ get_id e <> get_id e1 /\ mem e1
           (filter_op (fun e1 -> get_id e <> get_id e1 && ((tr.vis e e1) || (not (tr.vis e e1 || tr.vis e1 e) && (get_id e < get_id e1)))) (enq_list)) ==>
                          (memq ((get_id e), (get_ele e)) s0 /\ memq ((get_id e1), (get_ele e1)) s0 /\ get_id e <> get_id e1 /\
                                      order ((get_id e), (get_ele e)) ((get_id e1), (get_ele e1)) (tolist s0)
                                      ));
  ()
